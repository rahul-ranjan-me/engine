define([], 
	function()
	{
		...
		removeAlert = function(){
			var elem = document.getElementById('alerter'),
				pos = 25,
				alpha = .7,
			...
			function frame() {
				if (pos == 0) {
					...
					pos--;
					alpha = alpha -.05;
					elem.querySelector('.alert-bg').style.opacity = alpha;
					elem.querySelector('.ui-alert').style.top = pos + '%'; 
				}
			}
		},
	    ...
	    alerter = {
	    	buildAlert: function(arguments, type){
				...
				elem = document.getElementById('alerter');

				elem.innerHTML = '<div class="alert-bg"></div>'+createCommonDOM(content, type);
